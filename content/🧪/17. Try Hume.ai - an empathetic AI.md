...
- Context -
	- I have been on the lookout for a good conversational voice interface since I've been using ChatGPT specifically GPT-4's voice mode for the last 6 months, but the problem is that it always interrupts me and recently it's also been losing connection frequently, so I've been looking for alternatives. Sometimes the responses are also extremely long winded. So I was excited when my friend mentioned  Hume.ai as an alternative they just released their beta and so I decided to try their API.
- Answer -
	- Used Hume's typescript API alongside an Electron app
- Primarily for personal learning or novel discovery? - 
- What is the [[delta]]? - 
- Observations - 
	- I enjoyed using Hume's API overall. It's a streaming API with functions you can pass in upon interruption events, sending audio, and receiving audio back from the assistant.
	- I managed to get it working in 5-10 minutes, inside an Electron app. Easy setup.
	- I didn't give it any system prompt, just used the vanilla EVI (the name of the assistant). The assistant was surprisingly empathetic, although still very buggy. The typescript version demo had a lot of its audio cutting in and out between segments of assistant responses.
	...
	- Interactions with Hume were smoother than with ChatGPT, although not perfect. I was often interrupted when thinking about things, as I like to talk to this voice interface version of AI when I'm thinking through a problem. If I paused for more than a few seconds, Hume would interrupt.
	- Hume is not very good with background noise, similar to GPT-4. I've used GPT-4 in the car before, and it would interrupt me even more often than if I were in a quiet room. When I tried Hume with a friend at a coffee shop, it performed much worse than when I was using it in my quiet room. This seemed somewhat surprising since Whisper is pretty good with background noise. 
	- I found myself talking for quite a while with EVI! She's like that one really awkward friend who doesn't know how to keep the cadence of a conversation but has good intentions and knows a lot of random facts
